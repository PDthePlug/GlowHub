...
function legalRoute(hash:string){const path=routePath(hash);return path==='terms'||path==='privacy'?path as 'terms'|'privacy':null}
...
function Root(){
  ...
  const legal=legalRoute(hash),service=serviceRoute(hash),booking=bookingRoute(hash),waitlist=waitlistRoute(hash),profile=publicProfileRoute(hash);
  let content;
  if(isDiscoverHome(hash))content=<DiscoverHome/>;
  else if(isBusinessAuth(hash))content=<BusinessAuth/>;
  else if(legal)content=<LegalPages kind={legal}/>;
