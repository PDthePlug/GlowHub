...
function signedInNow(){try{return Boolean(localStorage.getItem(authKey()))}catch{return false}}
function authModal(){return new Promise<void>((resolve,reject)=>{
  ...
  const finish=()=>{wrap.remove();resolve()};const fail=(err:unknown)=>{const e=err as {message?:string};message.textContent=e.message||'Sign in could not be completed.';signIn.disabled=false;signUp.disabled=false};
