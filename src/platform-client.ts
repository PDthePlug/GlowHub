...
function ownerRoute(){return `${window.location.origin}${window.location.pathname}#/app`}
function forceOwnerRoute(){
  const target=ownerRoute();
  if(window.location.href!==target){history.replaceState({},'',target);window.dispatchEvent(new HashChangeEvent('hashchange'))}
}
function rawAuthFragment(){
  const raw=window.location.hash.replace(/^#/,'');
  const accessAt=raw.indexOf('access_token=');
  if(accessAt>=0)return raw.slice(accessAt);
  const errorAt=raw.indexOf('error=');
  if(errorAt>=0)return raw.slice(errorAt);
  return raw.replace(/^\/?/,'');
}
function looksLikeAuthFragment(raw:string){return /(^|[#&/?])(access_token|refresh_token|error|error_code|error_description|expires_in|token_type|type)=/i.test(raw)}

const incomingUrl=new URL(window.location.href);
const originalHash=window.location.hash.replace(/^#/,'');
const authFragment=rawAuthFragment();
const incomingCallback=new URLSearchParams(authFragment);
...
const hasAuthError=Boolean(incomingCallback.get('error')||incomingCallback.get('error_code')||incomingCallback.get('error_description'));
const hasAuthFragment=looksLikeAuthFragment(originalHash);
let callbackSessionPending=hasImplicitCallback||hasPkceCallback;

if(callbackSessionPending||hasAuthError||hasAuthFragment||incomingUrl.searchParams.get('auth')==='confirmation'){
...
if(hasImplicitCallback){
  authBootstrap=supabase.auth.setSession({access_token:callbackAccessToken,refresh_token:callbackRefreshToken}).then(({error})=>{if(error)throw error}).finally(()=>{callbackSessionPending=false;forceOwnerRoute()});
}else if(hasPkceCallback){
  authBootstrap=supabase.auth.exchangeCodeForSession(callbackCode).then(({error})=>{if(error)throw error}).finally(()=>{callbackSessionPending=false;forceOwnerRoute()});
}else if(hasAuthError||hasAuthFragment){
  callbackSessionPending=false;
  forceOwnerRoute();
}
...
async function request(method:Method,path:string,body?:unknown){
  await authBootstrap.catch(()=>undefined);
...
function authModal(){return new Promise<void>((resolve,reject)=>{
  ...
  const finish=()=>{forceOwnerRoute();wrap.remove();resolve()};
  const fail=(err:unknown)=>{const e=err as{message?:string};message.style.color='#9f3d45';message.textContent=e.message||'Sign in could not be completed.';signIn.disabled=false;signUp.disabled=false};
...
export const auth={isSignedIn:()=>signedInNow(),signIn:()=>authModal(),async signOut(){await authBootstrap.catch(()=>undefined);await supabase.auth.signOut()},client:supabase};
